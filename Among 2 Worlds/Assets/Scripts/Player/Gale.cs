...
public class Gale : MonoBehaviour     //manages abilities for Gale
{
    ...
    public void dash()
    {
        if ((Input.GetKey(KeyCode.LeftShift) || Input.GetButton("Dash")) && playerRef.dashused == false && playerRef.playerMoveState != Player.moveState.Walled && playerRef.currentDashTime <= playerRef.dashCooldown)
        {
